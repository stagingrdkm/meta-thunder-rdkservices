                                               ...
                                               GstBuffer* IV, GstBuffer* keyID, uint32_t initWithLast15)
{
    ...
    bool fullSampleEncrypted = false;
    if (session != nullptr) {
        ...
        if (mappedSubSample) {
            ...
            fullSampleEncrypted = true;
        }
        result = opencdm_session_decrypt(session, fullSampleEncrypted ? mappedData : encryptedData, bufferSize, mappedIV, mappedIVSize,
        ...
        uint32_t *tokenHandle = reinterpret_cast<uint32_t *>(fullSampleEncrypted ? mappedData : encryptedData);
        ...
        if (!block) {
            // we cannot clone - means we are in clearkey case
            if (!fullSampleEncrypted) {
                GstByteReader* reader = gst_byte_reader_new(mappedSubSample, mappedSubSampleSize);
